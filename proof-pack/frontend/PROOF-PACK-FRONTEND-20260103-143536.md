...
**제출일**: 2026-01-03  
**Commit Hash**: `4e106195552ea58cda28d45c7ac79ca52b1071f2`  
**담당**: Frontend+Envoy  
...
**비고**: Rate limiting 테스트는 백엔드 인증 단계에서 차단되어 401을 반환하고 있습니다. Public server에서 백엔드 인증 미들웨어를 우회하도록 설정한 후 재테스트 필요합니다.

### 4) 저장 증빙

**DB 확인 명령어** (Public server에서 실행 필요):
```bash
psql "$DATABASE_URL" -c "select id, left(email,3)||'***' as email_masked, created_at from waitlist order by created_at desc limit 1;"
```

**또는 백엔드 로그 확인** (Public server에서 실행 필요):
```bash
# 백엔드 로그에서 /api/waitlist POST 요청 확인
```

**비고**: 저장 증빙은 Public server에서 백엔드 로그 또는 DB를 통해 확인 필요합니다.
...
**비고**: Public waitlist 엔드포인트가 구현되어 있으며, rate limiting, input validation, spam 방지 기능이 포함되어 있습니다. 백엔드에서 인증 미들웨어를 우회하도록 설정하면 정상 동작할 것입니다.
...
- 실제 런타임 로그는 Public server에서 `docker logs limen-envoy --tail 200 | tail -n 30` 또는 `journalctl -u envoy -n 200 --no-pager`로 수집 필요
