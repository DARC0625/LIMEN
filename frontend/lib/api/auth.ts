...
export const authAPI = {
  ...
  login: async (credentials: LoginRequest): Promise<LoginResponse> => {
    ...
    console.log('[authAPI.login] Login response Set-Cookie headers:', {
    ...
    if (typeof window !== 'undefined') {
      ...
      for (const cookie of cookies) {
        ...
        if (name === 'refresh_token' && value) {
          ...
          console.log('[authAPI.login] Found refresh_token in cookie');
          break;
        }
      }
    }
    ...
    if (data.access_token && refreshToken) {
      ...
      console.log('[authAPI.login] Saving tokens to tokenManager', {
        hasAccessToken: !!data.access_token,
      ...
      console.log('[authAPI.login] Tokens saved, verifying...', {
  ...
  createSession: async (
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[authAPI.createSession] Creating session:', {
    ...
    try {
      ...
      const setCookieHeaders = response.headers.getSetCookie();
      if (process.env.NODE_ENV === 'development') {
        console.log('[authAPI.createSession] Session creation response Set-Cookie headers:', {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('[authAPI.createSession] Session created successfully:', result);
      }
    } catch (error) {
      console.error('[authAPI.createSession] Session creation failed:', error);
      throw error;
    }
  },
  ...
  refreshToken: async (refreshToken?: string): Promise<TokenRefreshResponse> => {
    ...
    if (!hasRefreshTokenCookie && refreshToken) {
      ...
      console.log('[authAPI.refreshToken] No refresh_token cookie found, using body');
    } else if (hasRefreshTokenCookie) {
      console.log('[authAPI.refreshToken] Using refresh_token from cookie');
    }

    console.log('[authAPI.refreshToken] Requesting token refresh...', {
    ...
    if (!response.ok) {
      ...
      console.error('[authAPI.refreshToken] Refresh failed:', {
        status: response.status,
    ...
    console.log('[authAPI.refreshToken] Refresh response:', {
      hasAccessToken: !!data.access_token,
      accessTokenLength: data.access_token?.length || 0,
      hasRefreshToken: !!data.refresh_token,
      expiresIn: data.expires_in,
    ...
    if (!data.access_token) {
      console.error('[authAPI.refreshToken] No access token in refresh response', data);
