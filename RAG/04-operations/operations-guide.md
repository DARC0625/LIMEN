...
2. **권한 확인**
```bash
ls -la /home/darc0/LIMEN/backend/server
...
2. **디스크 공간 확인**
```bash
df -h /home/darc0/LIMEN/database/vms
```
...
```bash
# VM 디렉토리 백업
tar -czf vm_backup_$(date +%Y%m%d).tar.gz /home/darc0/LIMEN/database/vms

# ISO 이미지 백업
tar -czf iso_backup_$(date +%Y%m%d).tar.gz /home/darc0/LIMEN/database/iso
```
...
# VM 이미지 백업
tar -czf "$BACKUP_DIR/vms_$DATE.tar.gz" /home/darc0/LIMEN/database/vms
