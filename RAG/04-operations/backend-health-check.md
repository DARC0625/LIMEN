...
```bash
# 최근 로그
tail -50 /home/darc0/LIMEN/backend/server.log

# 에러만 확인
tail -100 /home/darc0/LIMEN/backend/server.log | grep -E "(error|fatal|panic)"
```
...
echo ""
echo "4. 최근 에러:"
ERROR_COUNT=$(tail -100 /home/darc0/LIMEN/backend/server.log | grep -E "(error|fatal|panic)" | wc -l)
if [ "$ERROR_COUNT" -eq 0 ]; then
    echo "✅ 에러 없음"
else
    echo "⚠️  $ERROR_COUNT 개의 에러 발견"
    tail -100 /home/darc0/LIMEN/backend/server.log | grep -E "(error|fatal|panic)" | tail -5
fi
```
